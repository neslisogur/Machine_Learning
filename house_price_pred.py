...
train = pd.read_csv("miuul/machine_learning/house_price_prediction/house-prices/train.csv")
test = pd.read_csv("miuul/machine_learning/house_price_prediction/house-prices/test.csv")
...
#2- Numerik ve kategorik değişkenleri yakalayınız.
def grab_col_names(dataframe, cat_th=10, car_th=20):
    ...
    #eğer bir değişkenin içindeki sayısal değişkenin sınıf sayısı 10dan azsa kategoriktir.
    num_but_cat = [col for col in dataframe.columns if dataframe[col].nunique() < cat_th and
                   dataframe[col].dtypes != "O"]
    #eğer bir kategorik değişkenin 20den fazla sınıf sayısı varsa ve tipi de kategorikse cardinaldir.
    cat_but_car = [col for col in dataframe.columns if dataframe[col].nunique() > car_th and
                   dataframe[col].dtypes == "O"]
    cat_cols = cat_cols + num_but_cat
    #kategorik ama kardinal değişkenlerde olmayanları seç(kategorik, sayısal görünümlü kategorikleri topla kategorik ama kardinal olanları çıkar.)
    cat_cols = [col for col in cat_cols if col not in cat_but_car]

    # num_cols
    # int ya da floatları getir.
    num_cols = [col for col in dataframe.columns if dataframe[col].dtypes != "O"]
    # num_but_cat'leri çıkar
